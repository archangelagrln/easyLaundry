...
$pesanan = mysqli_query($connect, "SELECT id, tgl AS tanggal, layanan, berat, total_harga FROM pesanan $filterPesanan");
...
<body class="flex bg-gray-50 min-h-screen">
  ...
  <div class="ml-[45px] w-[calc(100%-212px)] h-screen overflow-y-auto p-6">
    ...
    <h1 style="font-size: 35px; color: #21B7E2;" class="text-center font-bold mb-8 mt-5">Order Report</h1>
      ...
      <table class="w-full text-sm text-left table-fixed border-collapse">
        <thead class="bg-[#D9D9D9] text-gray-800">
          <tr class="border-b-2 border-gray-500 ">
            ...
            <th class="py-3 px-4 text-center">Service</th>
        ...
        <tbody class="text-gray-700 bg-white">
          ...
          ?>
            <tr class="border-b border-gray-800">
              ...
              <td class="py-2 px-4 text-center"><?= $row['layanan'] ?></td>
              <td class="py-2 px-4 text-center"><?= $row['berat'] ?> Kg</td>
              <td class="py-2 px-4 text-center">Rp <?= number_format($row['total'], 0, ',', '.') ?></td>
            </tr>
          <?php endforeach; ?>


            <tr class="bg-[#F5F5F5] font-semibold border-t-2 border-gray-400">
              <td colspan="4" class="py-3 px-4 text-center">Total Order</td>
              <td class="py-3 px-4 text-center">Rp <?= number_format($totalOrder, 0, ',', '.') ?></td>
            </tr>
          </tbody>
        </table>
        </div>
